/**
 * This implimentation of a list is done in macros so it may be used generically
 * for any struct.
 *
 * To use, call DEFINE_LIST(typename) in the header file where you would want to
 * define the struct for the list.  typename should be the NON-TYPEDEFED name of
 * the struct without the keyword "struct".  If I wand to define a list of type
 * "struct point", I would need to do "DEFINE_LIST(point)".  This will create
 * the list type called "typename_list_t", which may be reused as many times as
 * you want.
 *
