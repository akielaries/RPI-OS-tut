...
/*
Name: removeFromMax
Process: recursively traverses for max node. When found, unlinks node from tree
and returns it. Function input/parameters: pointer to parent and child nodes
Function output/parameters: none
Function output/returned: pointer to unlinked node
...
PasswordType *remove_from_max(PasswordType *parentPtr, PasswordType *childPtr) {
    ...
    if (childPtr->rightChildPtr != NULL) {
        // call removeFromMax with the child as the parent parameter, and the
...
recursive level) Function input/parameters: rootPtr of tree, pointer to data
that should be removed Function output/parameters: none Function
...
PasswordType *remove_item_helper(PasswordType *wkgPtr,
    ...
    if (wkgPtr != NULL) {
        // check if the the data to be removed is less than the current node
        // data
        ...
        // otherwise, check to see if removal data is larger than the data at
        // the current node
        ...
        // otherwise, data to be removed has been found
            ...
            // check to see if the current node's (wkgPtr) left child is null
            ...
            // otherwise, check to see if the current node's right child is null
            ...
            // otherwise, assume node has two children
                ...
                // check to se if current node's left child has a right child
                if (wkgPtr->leftChildPtr->rightChildPtr != NULL) {
                    // call remove from max
                    temp = remove_from_max(wkgPtr,
                ...
                // otherwise, current node's left child does not have a right
                // child
                else {
...
/*
Name: removeLeafNode
Process: removes a leaf node from the tree, frees the removed leaf node from
memory Function input/parameters: wkgPtr (current node in tree), searchRank
(int, rank of the node that should be removed) Function output/parameters: none
Function output/returned: wkgPtr for recursion
Device input/file: none
Device output/monitor: none
Dependencies: free
*/
